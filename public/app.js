      ...
      if(statusCode == 200){
        ...
        app.client.request(undefined,'api/tokens','GET',queryStringObject,undefined,function(statusCode,responsePayload){
          ...
          if(statusCode == 200){
            ...
            app.setSessionToken(false);
            console.log("Error while getting updated token");
            callback(true);
          }
        });
      } else {
        app.setSessionToken(false);
         console.log("Error while getting updating token");
        callback(true);
      }
    });
  } else {
...
app.loadAccountEditPage = function(){
  // Get the phone_no number from the current token, or log the user out if none is there
  var phone_no = typeof(app.config.sessionToken.phone_no) == 'string' ? app.config.sessionToken.phone_no : false;
  if(phone_no){
    // Fetch the user data
    var queryStringObject = {
      'phone_no' : phone_no
    ...
    app.client.request(undefined,'api/users','GET',queryStringObject,undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
        ...
        document.querySelector("#accountEdit1 .displayPhoneInput").value = responsePayload.phone_no;

        // Put the htoken_idden phone_no field into both forms
       var hiddenPhoneInputs = document.querySelectorAll("input.hiddenPhoneNumberInput");
        for(var i = 0; i < hiddenPhoneInputs.length; i++){
            hiddenPhoneInputs[i].value = responsePayload.phone_no;
        }
